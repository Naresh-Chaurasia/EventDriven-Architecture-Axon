...
public class NotificationEventHandler {
    ...
    private NotificationEntity createNotificationEntity(PaymentSettledEvent event,
        ...
        // Explicitly set createdAt since @PrePersist only works for database
        // persistence
    ...
    public void on(PaymentRejectedEvent event) {
        ...
        if (useConsoleNotificationService) {
            // Use ConsoleNotificationService
            log.info("Using ConsoleNotificationService for rejected payment: {}", event.getPaymentId());
            ...
            consoleNotificationService.sendConsoleNotification(notification);
            log.info("Console notification sent for rejected payment: {}", event.getPaymentId());
            
        } else {
            // Use NotificationService
            log.info("Using NotificationService for rejected payment: {}", event.getPaymentId());
    ...
    private NotificationEntity createNotificationEntity(PaymentRejectedEvent event,
            Map<String, Object> notificationData) {
        // Create a proper NotificationEntity for console service
        NotificationEntity notification = new NotificationEntity();
        notification.setCorrelationId(event.getPaymentId());
        notification.setEventType("PaymentRejectedEvent");
        notification.setRecipient("user@" + event.getOrderId() + ".com");
        notification.setChannel(NotificationChannel.CONSOLE);
        notification.setStatus(NotificationStatus.SENT);
        notification.setSubject("Payment Rejected");
        ...
        content.append("- Payment ID: ").append(event.getPaymentId()).append("\n");
        content.append("- Order ID: ").append(event.getOrderId()).append("\n");
        content.append("- Rejection Reason: ").append(event.getRejectionReason()).append("\n");
        content.append("- Error Code: ").append(event.getErrorCode()).append("\n");
